...
inline void build(lv_obj_t* scr) {
    ...
    // Scrollable list
    lv_obj_t* list = lv_list_create(_overlay);
    lv_obj_set_size(list, CONTENT_W, CONTENT_H - 51);
    ...
    lv_obj_set_style_bg_color(list, lv_color_hex(C_BG), LV_PART_ITEMS);
    lv_obj_set_style_border_color(list, lv_color_hex(C_DIV), LV_PART_ITEMS);
    lv_obj_set_style_border_width(list, 0, LV_PART_ITEMS);
    lv_obj_set_style_border_side(list, LV_BORDER_SIDE_BOTTOM, LV_PART_ITEMS);
    lv_obj_set_style_pad_top(list, 8, LV_PART_ITEMS);
    lv_obj_set_style_pad_bottom(list, 8, LV_PART_ITEMS);
    lv_obj_set_style_pad_left(list, 20, LV_PART_ITEMS);

    uint32_t activeSat = SatTracker::getState().noradId;

    for (int g = 0; g < SAT_GROUP_COUNT; g++) {
        // Group header
        lv_obj_t* hdr = lv_list_add_text(list, SAT_GROUPS[g].label);
        ...
        for (int i = 0; i < SAT_GROUPS[g].count; i++) {
            ...
            // Load name from TLE file
            char name[30], l1[70], l2[70], display[32];
            if (TLEManager::loadTLE(id, name, l1, l2)) {
                ...
                snprintf(display, sizeof(display), "NORAD %lu", (unsigned long)id);
            }

            lv_obj_t* btn = lv_list_add_btn(list, nullptr, display);
            lv_obj_add_event_cb(btn, _sat_cb, LV_EVENT_CLICKED, (void*)(uintptr_t)id);

            // Font for the label inside the button
            lv_obj_t* lbl = lv_obj_get_child(btn, 0);
            if (lbl) lv_obj_set_style_text_font(lbl, &lv_font_montserrat_18, 0);
